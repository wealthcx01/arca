...
describe("GET /api/cards/:id", () => {
  ...
  // ARCA-53: CardDetailPage.tsx reads name, card_number, and image_url off the same
  // `{ data: card }` envelope. A regression that stops unwrapping it (or that drops a
  // column server-side) renders a blank identity/broken image silently, with no console
  // error — so these fields need their own assertions, not just set_name.
  test("a single card includes non-empty name, card_number, and image_url", async () => {
    const listRes = await app.request(
      `/api/cards?q=${encodeURIComponent("Test Card Alpha")}&limit=1`,
    );
    const listBody = await listRes.json();
    expect(listBody.data.length).toBeGreaterThan(0);
    const id = listBody.data[0].id;
  ...
  test("an unknown card id returns 404 with an error message", async () => {
