...
async def run():
    ...
    async with async_playwright() as p:
        ...
        print("\n[3] Attempting login...")
        try:
            # TRKD uses jQuery custom inputs: #input_uid.inputbox input, #input_pwd.inputbox input
            # The "Login" button is a div#button_login with jQuery .toolbutton() binding
            uid_sel = '#input_uid input[name="username"]'
            pwd_sel = '#input_pwd input[name="password"]'

            # Clear any placeholder text and fill credentials
            await page.fill(uid_sel, USERNAME)
            print(f"    Filled username via: {uid_sel}")
            await page.fill(pwd_sel, PASSWORD)
            print(f"    Filled password via: {pwd_sel}")

            # Trigger login via jQuery (the div#button_login uses jQuery .toolbutton() which calls form.submit())
            # First call the onsubmit handler to set the form action, then submit
            await page.evaluate("""
                onsubmitloginform();
                document.getElementById('loginform').submit();
            """)
            print("    Submitted form via JS (onsubmitloginform + submit)")
        ...
        try:
            await page.wait_for_url("**/tha_desktop/**", timeout=15_000)
            print("    URL changed after login")
        except Exception:
            print("    URL did not change — trying networkidle fallback")
        try:
        ...
        try:
            ...
            # Fallback: viewport-only screenshot (fonts may timeout on full_page)
            try:
                await page.screenshot(
                    path=str(OUTPUT_DIR / "screenshots/03_app_full.png"),
                    timeout=10_000,
                )
                print("    (viewport-only screenshot — full_page timed out)")
                ...
                try:
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            (OUTPUT_DIR / "js_bundles" / fname).write_text(content[:500_000], encoding="utf-8")
